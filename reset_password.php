...
if (isset($_GET['email']) && isset($_GET['reset_code'])) {
    ...
    $query = "SELECT * FROM `userdetails` WHERE `email`='$email' AND `reset_code`='$resetCode'";
        ...
        if (mysqli_num_rows($result) == 1) {
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($password === $confirmPassword) {
                    ...
                    $updateQuery = "UPDATE `userdetails` SET `password`='$hashedPassword', `reset_code`=NULL WHERE `email`='$email'";
...
<body>
    <div class="container ">
        <div class="card card-body" style="padding: 20px; width: 400px;">
            <h2 class="mt-0 mb-4">Reset Password</h2>
