...
if (isset($_POST['post_id'])) {
    ...
    $fetch_file_address_sql = $conn->prepare("SELECT fileaddress FROM postdetails WHERE id = ?");
    ...
    $file_address = $file_row['fileaddress'];
    ...
    $delete_comments_sql = $conn->prepare("DELETE FROM comments WHERE postid = ?");
    $delete_comments_sql->bind_param("i", $post_id);

    // Prepare SQL statement to delete the post from the database
    $delete_post_sql = $conn->prepare("DELETE FROM postdetails WHERE id = ?");
